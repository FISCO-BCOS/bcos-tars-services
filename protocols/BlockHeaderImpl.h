...
public:
  ...
  bcos::crypto::HashType const &txsRoot() const override { return *(reinterpret_cast<bcos::crypto::HashType *>(m_inner->txsRoot.data())); }
  bcos::crypto::HashType const &stateRoot() const override { return *(reinterpret_cast<const bcos::crypto::HashType *>(m_inner->stateRoot.data())); }
  bcos::crypto::HashType const &receiptsRoot() const override { return *(reinterpret_cast<const bcos::crypto::HashType *>(m_inner->receiptRoot.data())); }
  ...
  bcos::bytesConstRef extraData() const override { return bcos::bytesConstRef(m_inner->extraData.data(), m_inner->extraData.size()); }
  gsl::span<const bcos::protocol::Signature> signatureList() const override {
    return gsl::span(reinterpret_cast<const bcos::protocol::Signature *>(m_inner->signatureList.data()), m_inner->signatureList.size());
  }

  gsl::span<const uint64_t> consensusWeights() const override {
    return gsl::span(reinterpret_cast<uint64_t *>(m_inner->consensusWeights.data()), m_inner->consensusWeights.size());
  }
  ...
  void setParentInfo(bcos::protocol::ParentInfoList &&_parentInfo) override { setParentInfo(gsl::span(_parentInfo.data(), _parentInfo.size())); }

  void setTxsRoot(bcos::crypto::HashType const &_txsRoot) override { m_inner->txsRoot.assign(_txsRoot.begin(), _txsRoot.end()); }
  void setReceiptsRoot(bcos::crypto::HashType const& _receiptsRoot) override {m_inner->receiptRoot.assign(_receiptsRoot.begin(), _receiptsRoot.end());}
  void setStateRoot(bcos::crypto::HashType const &_stateRoot) override { m_inner->stateRoot.assign(_stateRoot.begin(), _stateRoot.end()); }
  void setNumber(bcos::protocol::BlockNumber _blockNumber) override { m_inner->blockNumber = _blockNumber; }
  ...
  void setSealerList(gsl::span<const bcos::bytes> const &_sealerList) override { m_inner->sealerList.assign(_sealerList.begin(), _sealerList.end()); }

  void setSealerList(std::vector<bcos::bytes> &&_sealerList) override { setSealerList(gsl::span(_sealerList.data(), _sealerList.size())); }

  void setConsensusWeights(gsl::span<const uint64_t> const &_weightList) override { m_inner->consensusWeights.assign(_weightList.begin(), _weightList.end()); }

  void setConsensusWeights(std::vector<uint64_t> &&_weightList) override { setConsensusWeights(gsl::span(_weightList.data(), _weightList.size())); }
...
public:
  ...
  bcos::protocol::BlockHeader::Ptr createBlockHeader() override { return std::make_shared<BlockHeaderImpl>(m_cryptoSuite); }
  bcos::protocol::BlockHeader::Ptr createBlockHeader(bcos::bytes const &_data) override { return createBlockHeader(bcos::ref(_data)); }
  ...
  virtual bcos::protocol::BlockHeader::Ptr createBlockHeader(bcos::protocol::BlockNumber _number) override {
    auto blockHeader = std::make_shared<BlockHeaderImpl>(m_cryptoSuite);
