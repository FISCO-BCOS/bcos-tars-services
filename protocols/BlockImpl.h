...
public:
  BlockImpl(bcos::protocol::TransactionFactory::Ptr _transactionFactory, bcos::protocol::TransactionReceiptFactory::Ptr _receiptFactory)
  ...
  bcos::protocol::BlockType blockType() const override { return (bcos::protocol::BlockType)m_inner.type; }

  // get blockHeader
  bcos::protocol::BlockHeader::Ptr blockHeader() override {
    return std::make_shared<bcostars::protocol::BlockHeaderImpl>(&(m_inner.blockHeader), m_transactionFactory->cryptoSuite());
  };

  bcos::protocol::Transaction::ConstPtr transaction(size_t _index) const override {
    return std::make_shared<const bcostars::protocol::TransactionImpl>(&(m_inner.transactions[_index]), m_transactionFactory->cryptoSuite());
  }

  bcos::protocol::TransactionReceipt::ConstPtr receipt(size_t _index) const override {
    return std::make_shared<bcostars::protocol::TransactionReceiptImpl>(&(m_inner.receipts[_index]), m_transactionFactory->cryptoSuite());
  };

  bcos::crypto::HashType const &transactionHash(size_t _index) const override {
    return *(reinterpret_cast<const bcos::crypto::HashType *>(m_inner.transactionsHash[_index].data()));
  }

  void setBlockType(bcos::protocol::BlockType _blockType) override { m_inner.type = (int32_t)_blockType; }

  // set blockHeader
  void setBlockHeader(bcos::protocol::BlockHeader::Ptr _blockHeader) override {
    m_inner.blockHeader = std::dynamic_pointer_cast<bcostars::protocol::BlockHeaderImpl>(_blockHeader)->inner();
  }

  void setTransaction(size_t _index, bcos::protocol::Transaction::Ptr _transaction) override {
    m_inner.transactions[_index] = std::dynamic_pointer_cast<bcostars::protocol::TransactionImpl>(_transaction)->inner();
  }
  void appendTransaction(bcos::protocol::Transaction::Ptr _transaction) override {
    m_inner.transactions.push_back(std::dynamic_pointer_cast<bcostars::protocol::TransactionImpl>(_transaction)->inner());
  }

  void setReceipt(size_t _index, bcos::protocol::TransactionReceipt::Ptr _receipt) override {
    m_inner.receipts[_index] = std::dynamic_pointer_cast<bcostars::protocol::TransactionReceiptImpl>(_receipt)->inner();
  }
  void appendReceipt(bcos::protocol::TransactionReceipt::Ptr _receipt) override {
    m_inner.receipts.push_back(std::dynamic_pointer_cast<bcostars::protocol::TransactionReceiptImpl>(_receipt)->inner());
  }

  void appendTransactionHash(bcos::crypto::HashType const &_txHash) override { m_inner.transactionsHash.emplace_back(_txHash.begin(), _txHash.end()); }
...
public:
  BlockFactoryImpl(bcos::crypto::CryptoSuite::Ptr cryptoSuite, bcos::protocol::BlockHeaderFactory::Ptr blockHeaderFactory,
                   bcos::protocol::TransactionFactory::Ptr transactionFactory, bcos::protocol::TransactionReceiptFactory::Ptr receiptFactory)
      : m_cryptoSuite(cryptoSuite), m_blockHeaderFactory(blockHeaderFactory), m_transactionFactory(transactionFactory), m_receiptFactory(receiptFactory){};

  ~BlockFactoryImpl() override {}
  bcos::protocol::Block::Ptr createBlock() override { return std::make_shared<BlockImpl>(m_transactionFactory, m_receiptFactory); }

  bcos::protocol::Block::Ptr createBlock(bcos::bytes const &_data, bool _calculateHash = true, bool _checkSig = true) override {
    return createBlock(bcos::ref(_data), _calculateHash, _checkSig);
  }

  bcos::protocol::Block::Ptr createBlock(bcos::bytesConstRef _data, bool _calculateHash = true, bool _checkSig = true) override {
  ...
  bcos::protocol::BlockHeaderFactory::Ptr blockHeaderFactory() override { return m_blockHeaderFactory; }
  bcos::protocol::TransactionFactory::Ptr transactionFactory() override { return m_transactionFactory; }
  bcos::protocol::TransactionReceiptFactory::Ptr receiptFactory() override { return m_receiptFactory; }
