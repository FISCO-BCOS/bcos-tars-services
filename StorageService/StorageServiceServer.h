...
public:
  ...
  bcostars::Error addStateCache(tars::Int64 blockNumber, const bcostars::TableFactory &tableFactory, tars::TarsCurrentPtr current) override {
    try {
  ...
  bcostars::Error commitBlock(tars::Int64 blockNumber, const vector<bcostars::TableInfo> &infos, const vector<map<std::string, bcostars::Entry>> &datas,
    ...
    for (auto const &tableInfo : infos) {
      bcosTableInfos.emplace_back(std::make_shared<bcos::storage::TableInfo>(tableInfo.name, tableInfo._key, tableInfo.fields));
    }

    auto bcosDatas = std::vector<std::shared_ptr<std::map<std::string, bcos::storage::Entry::Ptr>>>();
  ...
  bcostars::Error get(const std::string &columnFamily, const std::string &_key, std::string &value, tars::TarsCurrentPtr current) override {
    try {
  ...
  bcostars::Error getBatch(const std::string &columnFamily, const vector<std::string> &keys, vector<std::string> &values,
                           tars::TarsCurrentPtr current) override {
    current->setResponse(false);

    m_storage->asyncGetBatch(columnFamily, std::make_shared<vector<std::string>>(keys),
                             [current, this](bcos::Error::Ptr error, std::shared_ptr<std::vector<std::string>> values) {
                               async_response_getBatch(current, toTarsError(error), *values);
                             });
  }

  bcostars::Error getPrimaryKeys(const bcostars::TableInfo &tableInfo, const bcostars::Condition &condition, vector<std::string> &keys,
                                 tars::TarsCurrentPtr current) override {
  ...
  bcostars::Error getRow(const bcostars::TableInfo &tableInfo, const std::string &key, bcostars::Entry &row, tars::TarsCurrentPtr current) override {
    try {
      auto bcosTableInfo = toBcosTableInfo(tableInfo);
  ...
  bcostars::Error getRows(const bcostars::TableInfo &tableInfo, const vector<std::string> &keys, map<std::string, bcostars::Entry> &rows,
                          tars::TarsCurrentPtr current) override {
    try {
  ...
  bcostars::Error getStateCache(tars::Int64 blockNumber, bcostars::TableFactory &tableFactory, tars::TarsCurrentPtr current) override {
    try {
  ...
  bcostars::Error put(const std::string &columnFamily, const std::string &_key, const std::string &value, tars::TarsCurrentPtr current) override {
    try {
  ...
  bcostars::Error remove(const std::string &columnFamily, const std::string &_key, tars::TarsCurrentPtr current) override {
    try {
