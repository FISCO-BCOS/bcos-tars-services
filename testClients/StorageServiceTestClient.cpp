...
auto now() {
  return std::make_tuple(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count(), clock());
}
...
void testSyncPut(
    const bcos::storage::StorageInterface::Ptr &client,
...
std::string genRandomString(size_t length) {
  ...
  std::mt19937 random_engine(rng() + std::chrono::system_clock::now().time_since_epoch().count() * rng());
