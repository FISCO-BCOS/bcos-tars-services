...
public:
  ...
  void asyncExecuteBlock(const bcos::protocol::Block::Ptr &_block, bool _verify,
                         std::function<void(const bcos::Error::Ptr &, const bcos::protocol::BlockHeader::Ptr &)> _callback) override {
    class Callback : public bcostars::DispatcherServicePrxCallback {
    public:
      Callback(std::function<void(const bcos::Error::Ptr &, const bcos::protocol::BlockHeader::Ptr &)> callback,
      ...
      void callback_asyncExecuteBlock(const bcostars::Error &ret, const bcostars::BlockHeader &blockHeader) override {
      ...
      void callback_asyncExecuteBlock_exception(tars::Int32 ret) override { m_callback(toBcosError(ret), nullptr); }

    private:
      std::function<void(const bcos::Error::Ptr &, const bcos::protocol::BlockHeader::Ptr &)> m_callback;
      bcostars::protocol::BlockHeaderFactoryImpl::Ptr m_blockHeaderFactory;
    };

    m_proxy->async_asyncExecuteBlock(new Callback(_callback, m_blockHeaderFactory), std::dynamic_pointer_cast<bcostars::protocol::BlockImpl>(_block)->inner(),
                                     _verify);
  }

  void asyncGetLatestBlock(std::function<void(const bcos::Error::Ptr &, const bcos::protocol::Block::Ptr &)> _callback) override {
    class Callback : public bcostars::DispatcherServicePrxCallback {
    public:
      Callback(std::function<void(const bcos::Error::Ptr &, const bcos::protocol::Block::Ptr &)> callback,
               bcostars::protocol::BlockFactoryImpl::Ptr blockFactory)
          : m_callback(callback), m_blockFactory(blockFactory) {}

      void callback_asyncGetLatestBlock(const bcostars::Error &ret, const bcostars::Block &block) override {
      ...
      void callback_asyncGetLatestBlock_exception(tars::Int32 ret) override { m_callback(toBcosError(ret), nullptr); }

    private:
      std::function<void(const bcos::Error::Ptr &, const bcos::protocol::Block::Ptr &)> m_callback;
  ...
  void asyncNotifyExecutionResult(const bcos::Error::Ptr &_error, const bcos::protocol::BlockHeader::Ptr &_header,
    ...
    public:
      ...
      void callback_asyncNotifyExecutionResult(const bcostars::Error &ret) override { m_callback(toBcosError(ret)); }

      void callback_asyncNotifyExecutionResult_exception(tars::Int32 ret) override { m_callback(toBcosError(ret)); }
