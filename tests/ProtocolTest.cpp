...
struct Fixture {
  Fixture() {
    cryptoSuite = std::make_shared<bcos::crypto::CryptoSuite>(
        std::make_shared<bcos::test::Sm3Hash>(),
...
BOOST_AUTO_TEST_CASE(transactionReceipt) {
  ...
  BOOST_CHECK_EQUAL(bcos::asString(receipt->contractAddress()),
                    bcos::asString(contractAddress));
  BOOST_CHECK_EQUAL(receipt->logEntries().size(), logEntries->size());
  for (auto i = 0; i < receipt->logEntries().size(); ++i) {
    BOOST_CHECK_EQUAL(receipt->logEntries()[i].address().toString(),
                      (*logEntries)[i].address().toString());
    BOOST_CHECK_EQUAL(receipt->logEntries()[i].topics().size(),
                      (*logEntries)[i].topics().size());
    for (auto j = 0; j < receipt->logEntries()[i].topics().size(); ++j) {
      BOOST_CHECK_EQUAL(receipt->logEntries()[i].topics()[j].hex(),
                        (*logEntries)[i].topics()[j].hex());
    }
    BOOST_CHECK_EQUAL(receipt->logEntries()[i].data().toString(),
                      (*logEntries)[i].data().toString());
  }
...
BOOST_AUTO_TEST_CASE(block) {
  ...
  BOOST_CHECK_EQUAL(block->transactionsSize(),
                    decodedBlock->transactionsSize());
  ...
  for (size_t i = 0; i < block->receiptsSize(); ++i) {
    ...
    BOOST_CHECK_EQUAL(bcos::asString(lhs->contractAddress()),
                      bcos::asString(rhs->contractAddress()));
    ...
    for (auto i = 0; i < lhs->logEntries().size(); ++i) {
      ...
      BOOST_CHECK_EQUAL(lhs->logEntries()[i].topics().size(),
                        rhs->logEntries()[i].topics().size());
      ...
      BOOST_CHECK_EQUAL(lhs->logEntries()[i].data().toString(),
                        rhs->logEntries()[i].data().toString());
    }

    BOOST_CHECK_EQUAL(lhs->status(), rhs->status());
    BOOST_CHECK_EQUAL(bcos::asString(lhs->output()),
                      bcos::asString(rhs->output()));
