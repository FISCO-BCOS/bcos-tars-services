...
public:
  ...
  void asyncGetNodeIDs(bcos::front::GetNodeIDsFunc _getNodeIDsFunc) override {
    ...
    public:
      ...
      void callback_asyncGetNodeIDs(
          ...
          const vector<vector<tars::UInt8>> &nodeIDs) override {
        auto bcosNodeIDs =
            std::make_shared<std::vector<bcos::crypto::NodeIDPtr>>();
  ...
  onReceiveNodeIDs(const std::string &_groupID,
    ...
    m_proxy->async_onReceivedNodeIDs(new Callback(_receiveMsgCallback),
                                     _groupID, tarsNodeIDs);
  }
  ...
  void onReceiveBroadcastMessage(
    ...
    public:
      ...
      void
      callback_onReceiveBroadcastMessage(const bcostars::Error &ret) override {
        m_callback(toBcosError(ret));
      }

      void
      callback_onReceiveBroadcastMessage_exception(tars::Int32 ret) override {
        m_callback(toBcosError(ret));
      }
    ...
    m_proxy->async_onReceiveBroadcastMessage(new Callback(_receiveMsgCallback),
                                             _groupID, _nodeID->data(),
                                             _data.toBytes());
  }
  ...
  void asyncSendMessageByNodeID(int _moduleID, bcos::crypto::NodeIDPtr _nodeID,
    ...
    public:
      ...
      void callback_asyncSendMessageByNodeID(
          const bcostars::Error &ret, const vector<tars::UInt8> &responseNodeID,
          const vector<tars::UInt8> &responseData,
      ...
      void
      callback_asyncSendMessageByNodeID_exception(tars::Int32 ret) override {
    ...
    m_proxy->async_asyncSendMessageByNodeID(new Callback(_callback, this),
                                            _moduleID, _nodeID->data(),
                                            _data.toBytes(), _timeout);
  }

  void asyncSendResponse(const std::string &_id,
                         bcos::bytesConstRef _data) override {
    m_proxy->async_asyncSendResponse(nullptr, _id, _data.toBytes());
  }
  ...
  void asyncSendMessageByNodeIDs(
    ...
    m_proxy->async_asyncSendMessageByNodeIDs(nullptr, _moduleID, tarsNodeIDs,
                                             _data.toBytes());
  }

  void asyncSendBroadcastMessage(int _moduleID,
                                 bcos::bytesConstRef _data) override {
    m_proxy->async_asyncSendBroadcastMessage(nullptr, _moduleID,
                                             _data.toBytes());
  }
