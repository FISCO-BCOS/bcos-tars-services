...
public:
  bcostars::Error asyncFillBlock(const vector<vector<tars::UInt8>> &txHashs, vector<bcostars::Transaction> &filled, tars::TarsCurrentPtr current) override {
    ...
    m_txpool->asyncFillBlock(hashList, [current](bcos::Error::Ptr error, bcos::protocol::TransactionsPtr txs) {
      std::vector<bcostars::Transaction> txList;
      for (auto tx : *txs) {
        txList.push_back(std::dynamic_pointer_cast<bcostars::protocol::TransactionImpl>(tx)->inner());
      }
  ...
  bcostars::Error asyncMarkTxs(const vector<vector<tars::UInt8>> &txHashs, tars::Bool sealedFlag, tars::TarsCurrentPtr current) override {
    current->setResponse(false);
    auto hashList = std::make_shared<std::vector<bcos::crypto::HashType>>();
    for (auto hashData : txHashs) {
      hashList->push_back(bcos::crypto::HashType(hashData));
    ...
    m_txpool->asyncMarkTxs(hashList, sealedFlag, [current](bcos::Error::Ptr error) { async_response_asyncMarkTxs(current, toTarsError(error)); });
    return bcostars::Error();
  }

  bcostars::Error asyncNotifyBlockResult(tars::Int64 blockNumber, const vector<bcostars::TransactionSubmitResult> &result,
    ...
    m_txpool->asyncNotifyBlockResult(blockNumber, bcosResultList,
                                     [current](bcos::Error::Ptr error) { async_response_asyncNotifyBlockResult(current, toTarsError(error)); });
    return bcostars::Error();
  }

  bcostars::Error asyncNotifyTxsSyncMessage(const bcostars::Error &error, const vector<tars::UInt8> &nodeID, const vector<tars::UInt8> &data,
                                            vector<tars::UInt8> &respData, tars::TarsCurrentPtr current) override {
    current->setResponse(false);

    auto bcosNodeID = m_cryptoSuite->keyFactory()->createKey(nodeID);

    m_txpool->asyncNotifyTxsSyncMessage(
        toBcosError(error), bcosNodeID, bcos::ref(data),
        [current](bcos::bytesConstRef response) { async_response_asyncNotifyTxsSyncMessage(current, toTarsError(nullptr), response.toBytes()); },
        [](bcos::Error::Ptr) {});

    return bcostars::Error();
  }

  bcostars::Error asyncSealTxs(tars::Int64 txsLimit, const vector<vector<tars::UInt8>> &avoidTxs, vector<vector<tars::UInt8>> &return1,
                               vector<vector<tars::UInt8>> &return2, tars::TarsCurrentPtr current) override {
    current->setResponse(false);
    ...
    m_txpool->asyncSealTxs(txsLimit, bcosAvoidTxs, [current](bcos::Error::Ptr error, bcos::crypto::HashListPtr list1, bcos::crypto::HashListPtr list2) {
  ...
  bcostars::Error asyncSubmit(const vector<tars::UInt8> &tx, bcostars::TransactionSubmitResult &result, tars::TarsCurrentPtr current) override {
    current->setResponse(false);
  ...
  bcostars::Error asyncVerifyBlock(const vector<tars::UInt8> &generatedNodeID, const vector<tars::UInt8> &block, tars::Bool &result,
                                   tars::TarsCurrentPtr current) override {
    current->setResponse(false);

    bcos::crypto::PublicPtr pk = m_cryptoSuite->keyFactory()->createKey(generatedNodeID);
    m_txpool->asyncVerifyBlock(pk, bcos::ref(block),
                               [current](bcos::Error::Ptr error, bool result) { async_response_asyncVerifyBlock(current, toTarsError(error), result); });
